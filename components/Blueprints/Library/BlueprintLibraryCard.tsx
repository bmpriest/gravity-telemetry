...
export default function BlueprintLibraryCard({ ship, variantCount, onClick }: Props) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group relative flex h-56 w-full flex-col overflow-hidden rounded-2xl bg-neutral-100/40 p-2 text-left transition duration-300 hover:bg-neutral-200 hover:shadow-lg dark:bg-neutral-900 dark:hover:bg-neutral-800"
    >
      {/* CP + RE chips */}
      <div className="absolute right-2 top-3 z-10 flex flex-col items-end gap-1">
      ...
      <div className="flex items-center gap-2">
        {ship.manufacturer.logo && (
          <img className="size-8 shrink-0 select-none rounded object-contain invert dark:invert-0" src={ship.manufacturer.logo} alt={ship.manufacturer.name} title={ship.manufacturer.name} />
        )}
      ...
      {variantCount > 1 && (
        <div className="absolute left-2 bottom-2 z-10 rounded-md bg-neutral-200/80 px-1.5 py-0.5 text-[0.65rem] font-medium text-neutral-700 transition duration-500 dark:bg-neutral-700/80 dark:text-neutral-200">
