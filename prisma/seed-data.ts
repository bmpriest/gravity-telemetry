...
/**
 * Snapshot of ship data, exported here as the one-time bootstrap source for `npm run db:seed`.
 * The runtime types in `utils/ships.ts` no longer include `direction`/`scope`/`weight`; those
 * fields remain present here so the file can be hand-edited and re-seeded, but the seed script
 * strips them when loading rows into the database.
 */
type SeedShip = AllShip & {
  direction?: readonly string[];
  scope?: string;
  weight?: number;
};

export const data: readonly SeedShip[] = [
    {
      id: 0,
      name: "AT021",
